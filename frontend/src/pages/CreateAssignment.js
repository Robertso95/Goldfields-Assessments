...
const CreateAssignment = () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (assignmentData) {
          ...
          // Set state values
          setSelectedStudents(
            Array.isArray(assignmentData.studentNames)
              ? assignmentData.studentNames
              : [assignmentData.studentNames]
          );
  ...
  useEffect(() => {
    const storedStudentId =
      sessionStorage.getItem("selectedStudentId") || preSelectedStudentId;
    const storedClassId =
      sessionStorage.getItem("selectedClassId") || preSelectedClassId;
  ...
  useEffect(() => {
    const fetchReferenceData = async () => {
      try {
        const [learningSetsRes, assignmentTypesRes, tagsRes] =
          await Promise.all([
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      console.error(
        `Error ${isEditing ? "updating" : "creating"} assessment:`,
        error
      );
      ...
      const errorMsg =
        ...
        error.message;
      message.error(
        `Failed to ${isEditing ? "update" : "create"} assessment: ${errorMsg}`
      );
    } finally {
      setLoading(false);
    }
        ...
        <Spin spinning={loading}>
          ...
          >
            <Form.Item
              ...
              name="className"
              rules={[
                { required: true, message: "Please select a class name!" },
              ]}
            >
              <Select
            ...
            <Form.Item
              ...
              name="subject"
              rules={[
                { required: !isEditing, message: "Please select the title!" },
              ]}
            >
              <Select
